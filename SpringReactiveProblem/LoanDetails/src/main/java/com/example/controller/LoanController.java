...
public class LoanController {
	...
	public Mono<ResponseEntity<ResponseMessage>> applyLoan(@RequestBody Loan loan) throws URISyntaxException {
		...
		try {
			...
			return Mono.just(ResponseEntity.created(location)
					.body(this.getResponse(loan.getLoanId(), "Loan Applied!!!", HttpStatus.OK)));
		} catch (Exception ex) {
			return Mono.just(ResponseEntity.created(location).body(
					this.getResponse(loan.getLoanId(), "Failed to Apply Loan!!!", HttpStatus.INTERNAL_SERVER_ERROR)));
	...
	private ResponseMessage getResponse(Integer id, String message, HttpStatus status) {
